...
export default function main(ts类型文件路径: string) {
    ...
    var r = {
        async _构建缓存() {
            ...
            var program = TJS.getProgramFromFiles([resolve(__dirname, ts类型文件路径)], { strictNullChecks: true })
            var 文件筛选 = program
                .getSourceFiles()
                .map((a) => a.fileName)
        ...
        async 构建缓存() {
            ...
            var 存在 = await new Promise((res) => {
                fs.exists(文件缓存路径, (data) => res(data))
            })
            if (存在) {
